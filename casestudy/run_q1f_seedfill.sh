#!/usr/bin/env bash
# Q1f: seed fill to n=6 on the five non-primary scenes -- seeds {5150,6001,7777},
# baseline & ours (5000 it) -> 30 runs. After this every scene-region cell of the
# paired table (T2) is evaluated at the protocol's stringent six-seed tier, and the
# IARPA_003 weak-proxy cell (the one sign-consistent n=3 cell) is resolved either way.
set -Eeuo pipefail
...
LOG=$LOGDIR/q1f_$(date +%Y%m%d_%H%M%S).log
ln -sfn "$LOG" "$LOGDIR/q1f_latest.log"
exec > >(tee -a "$LOG") 2>&1
...
echo "===== Q1F SEED FILL START $(date '+%F %T') ====="
for seed in 5150 6001 7777; do
  for scene in JAX_004 JAX_068 IARPA_001 IARPA_002 IARPA_003; do
    clear_evidence
